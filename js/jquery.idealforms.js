...
$.idealforms.errors = {
  ...
  required: '此处是必填滴，tanks',
  number: '必须是数字.',
  digits: '必须是唯一的数字.',
  name: '必须至少有3个字符长，并且只能包含字母.',
  username: '两个字符以上,只能有中文,字母,数字,下划线',
    codeImage:'请输入完整的验证码',
    registerCode:'请输入完整的注册码',
  pass: '必须至6到25个字符长，并且至少包含一个数字，一个大写字母和一个小写字母.',
  strongpass: '必须至少为8个字符长，至少包含一个大写字母和一个小写字母和一个数字或特殊字符.',
  email: '必须是一个有效的e -mail地址. <em>(e.g. user@520it.com)</em>',
...
var getFilters = function() {
  ...
  var filters = {
    ...
    username: {
      regex: /^[\u4e00-\u9fa5A-Za-z0-9_]{2,20}$/i,
      error: $.idealforms.errors.username
    },

      codeImage: {
          regex: /^[a-z0-9]{4}$/i,
          error: $.idealforms.errors.codeImage
      },

      registerCode: {
          regex: /^[a-z0-9A-Z]{6}$/i,
          error: $.idealforms.errors.registerCode
      },
...
var _defaults = {
  ...
  customFlags: {},
  globalFlags: '',
  onSuccess: function(e) { alert('正在提交！请耐性等待返回的结果，请勿刷新！！') },
